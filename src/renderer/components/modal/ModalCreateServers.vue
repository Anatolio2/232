    ...
    <div id="app">
      ...
      <div class="modal-overlay"   @click.self="$emit('close-modal')">
        <div class="modal">
          ...
          <div class="separator" v-if="this.steps == 0"></div>
  
          <!-- Начните с шаблона -->
          <div class="list" v-if="this.steps == 0">
            <h4 class="section-title">Начните с шаблона</h4>
            <div
              class="list-item"
            ...
            >
              <span>
                <img :src="option.icon" alt="Иконка" />
                {{ option.text }}
              </span>
              <span>&#8250;</span>
            </div>
          ...
          <div class="modal-footer" v-if="this.steps == 0">
            <div class="bottom-text">У вас уже есть приглашение?</div>
            <button class="join-button" @click="joinServer">Присоединиться к серверу</button>
          </div>

          <div class="modal-footer" v-if="this.steps == 1">
            <button class="modal-button" 
            style="background-color: transparent;"
            @click="prev()">Назад</button>
            <button class="modal-button" @click="next()">Создать</button>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        options: [
          {
            icon: "https://via.placeholder.com/24/FF5722/FFFFFF?text=G", // Иконка "Игры"
            text: "Игры",
          },
          {
            icon: "https://via.placeholder.com/24/3F51B5/FFFFFF?text=S", // Иконка "Школьный клуб"
            text: "Школьный клуб",
          },
          {
            icon: "https://via.placeholder.com/24/FFC107/FFFFFF?text=U", // Иконка "Учебная группа"
            text: "Учебная группа",
          },
        ],
      };
    ...
    methods: {
      next(){
        this.steps == 1 ? this.steps++ : this.steps--;
        if (this.steps == 2) {
     //   console.log("Server info before sending:", this.serverInfo); // Логируем перед отправкой

        /* const formData = new FormData();
        formData.append('token', this.serverInfo.token);
        formData.append('name', this.serverInfo.name);
        formData.append('avatar', this.serverInfo.avatarFile);  */
 ...
 */
        ...
        window.GoidaZvonAPI.createServer({
          token: this.serverInfo.token,
          name: this.serverInfo.name,
          avatarUrl: this.serverInfo.imageUrl, // Используем URL для изображения
      });
        this.closeModal();
    }
      },
      prev(){
        this.steps == 0 ? this.steps++ : this.steps--
      ...
      handleFileChange(event) {
        ...
        if (file && file.type.startsWith('image/')) {  // Проверка на тип файла
          const reader = new FileReader();

          // Когда файл прочитан, обновляем imageUrl для предварительного просмотра
          reader.onload = (e) => {
           
            this.serverInfo.imageUrl = e.target.result;  // Сохраняем URL изображения для отображения
          };

          reader.readAsDataURL(file);  // Преобразуем файл в формат base64 для отображения
          this.serverInfo.avatarFile = file;  // Сохраняем сам файл в serverInfo
        } else {
          alert('Пожалуйста, выберите изображение формата PNG, JPG или JPEG.');
        }
      },
